...
def _extract_chunked_file(fe: FileEntry, chunk_manifest: dict, step: ProcessStepDef,
    ...
    # Per-chunk flag determined here so the legend presence check can use it.
    # Legend extraction (array sub-steps) processes each chunk individually; stitching
    # a 5000px-wide panorama makes the small legend panel unreadable by the vision model.
    _use_per_chunk = bool(_array_sub_steps)
    ...
    _SYMBOL_REF: dict = {
        ...
        # Additional entries present on D526458 drawings — added to Phase 1.3 gap-fill
        ...
        # ARP5495 drawing entries — confirmed missing when Phase 1 vision returns blank entries
