# Agent 07 — Reference Architecture

## Overview

Agent 07 is a **document intelligence pipeline** for processing Customer Connections Applications (CCAs) in an electricity distribution engineering context (domain: Endeavour Energy project reviews). The system ingests a folder of project documents (PDFs, drawings, spreadsheets), classifies and extracts structured engineering and funding information from each, validates every step's output against quality criteria, and produces a consolidated review report.

Agent 07 extends Agent 06 with a 10-step pipeline (up from 7), a dedicated Data Analytics Agent, multi-phase legend extraction with quadrant retry, canonical symbol normalisation via `_SYMBOL_REF`, deterministic (Python-built) asset registers, and five critical bug fixes that enabled the first successful end-to-end experiment runs.
...
## Critical Bug Fixes (Agent 06 → Agent 07)
...
| BUG-04 | `orchestrator.py` | String asset IDs stored without bare-ID normalisation | IDs with leading zeros (e.g., `"02002180"`) or alpha prefixes (`"CLMN02002180"`) failed to match normalised bare-ID set |
