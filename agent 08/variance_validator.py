...
# Threshold above which two long text values are considered equivalent.
# Applied only to strings longer than _FUZZY_MIN_LEN characters.
_FUZZY_SIMILARITY = 0.97
_FUZZY_MIN_LEN    = 80


def _canonical(norm_values: List[str]) -> List[str]:
    """Replace each value with the representative of its similarity cluster.

    For short strings (structured data, IDs, numbers) exact equality is used.
    For long strings (multi-paragraph notes) two values are considered the same
    if their SequenceMatcher ratio >= _FUZZY_SIMILARITY — minor punctuation
    variants, trailing conjunctions, and mid-sentence line-break differences
    are absorbed without masking genuine content disagreements.

    Returns a new list of the same length where all members of a cluster share
    the same representative string (the first member encountered).
    """
    import difflib
    canonical = list(norm_values)
    for i, a in enumerate(norm_values):
        if len(a) < _FUZZY_MIN_LEN:
            continue  # short field — keep exact matching
        for j in range(i + 1, len(norm_values)):
            b = norm_values[j]
            if canonical[j] == canonical[i]:
                continue  # already in the same cluster
            if len(b) < _FUZZY_MIN_LEN:
                continue
            ratio = difflib.SequenceMatcher(None, a, b, autojunk=False).ratio()
            if ratio >= _FUZZY_SIMILARITY:
                canonical[j] = canonical[i]  # merge j into i's cluster
    return canonical
...
def _compute_variance(run_reports: List[dict]) -> dict:
        ...
        for extraction in report.get("extractions", []):
            ...
            if not raw_sse:
                ...
                if se:
                    ...
                    # Keys: "asset.<asset_id>.<field>", e.g. "asset.SLGT00316949.model"
    ...
    for fname, runs in file_runs.items():
        ...
        for key in sorted(all_keys):
            ...
            # Fuzzy-clustered values: long text fields use similarity grouping so
            # minor punctuation/conjunction variants don't register as unique values.
            fuzzy_values = _canonical(norm_values)

            # Unique values preserving first-seen order (fuzzy-aware)
            unique_vals = list(dict.fromkeys(fuzzy_values))
            most_common = max(set(fuzzy_values), key=fuzzy_values.count)
            match_rate  = fuzzy_values.count(most_common) / len(fuzzy_values)
            is_consistent = len(unique_vals) == 1

            # If fuzzy clustering changed anything, note how many exact variants existed
            exact_unique = list(dict.fromkeys(norm_values))
            fuzzy_absorbed = len(exact_unique) - len(unique_vals)
            ...
            field_analysis[key] = {
                ...
                "fuzzy_variants_absorbed": fuzzy_absorbed if fuzzy_absorbed > 0 else None,
                "values_by_run": {
