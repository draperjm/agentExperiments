...
def _generate(prompt: str) -> tuple:
    """Try Gemini first, fall back to OpenAI. Returns (response_text, model_used)."""
    try:
        if google_key:
            return _call_gemini(prompt), "gemini-2.5-flash"
    except Exception as e:
        print(f"[VALIDATOR] Gemini failed: {e}, falling back to OpenAI...")

    if openai_client:
        return _call_openai(prompt), "gpt-4o"

    raise RuntimeError("No AI provider available (set GOOGLE_API_KEY or OPENAI_API_KEY)")
