# Agent 09 — Extraction Consistency Experiment Report

**Date:** 2026-05-03
**Pipeline:** Customer Connections Application Review (tmpl-jr-review-customer-connections)
**Experiments:** Data Set 1, Data Set 2, Data Set 4, Data Set 5, Data Set 6
...
|---|---|---:|---:|---|
| Data Set 1 | Extract Site Plan Information | 10 | 100.0% | ✓ PASS |
| Data Set 1 | Extract Drawing Legend | 10 | 100.0% | ✓ PASS |
| Data Set 1 | Extract Asset Spreadsheet Data | 10 | 99.5% | ✓ PASS |
| Data Set 2 | Extract Site Plan Information | 10 | 100.0% | ✓ PASS |
| Data Set 2 | Extract Drawing Legend | 10 | 100.0% | ✓ PASS |
| Data Set 2 | Extract Asset Spreadsheet Data | 10 | 97.4% | ✓ PASS |
| Data Set 4 | Extract Site Plan Information | 10 | 100.0% | ✓ PASS |
| Data Set 4 | Extract Drawing Legend | 10 | 100.0% | ✓ PASS |
| Data Set 4 | Extract Asset Spreadsheet Data | 10 | 99.7% | ✓ PASS |
| Data Set 5 | Extract Site Plan Information | 10 | 98.2% | ✓ PASS |
| Data Set 5 | Extract Drawing Legend | 10 | 100.0% | ✓ PASS |
| Data Set 5 | Extract Asset Spreadsheet Data | 10 | 99.1% | ✓ PASS |
| Data Set 6 | Extract Site Plan Information | 10 | 100.0% | ✓ PASS |
| Data Set 6 | Extract Drawing Legend | 10 | 100.0% | ✓ PASS |
| Data Set 6 | Extract Asset Spreadsheet Data | 10 | 93.3% | ✓ PASS |

---

## Step 5 — Extract Site Plan Information

### Data Set 1 · A534927_ARP5495_RETIC.pdf
...
### Data Set 2 · D526458_ARP4983_RETIC_NOTES.pdf

**Score: 100% (11/11 fields consistent across 10 runs)**

Identical result to Data Set 1. All 11 fields consistent across all 10 runs with a single unique value each.

### Data Set 4 · A524058_NRL15205_NOTES.pdf
...
### Data Set 5 · A526924_NRL15237_RETIC_NOTES.pdf
...
### Data Set 6 · A535774_URS35318_RETIC_NOTES.pdf
...
### Data Set 1 · ARP5495_TAL_202503141.XLSM
...
|---|---|---|---|
| SLGT00316953 | status | 50% | `X` (runs 1–4, 6), `Marked for Deletion` (runs 5, 7–9), `Flagged for Deletion` (run 10) |
| SLGT00316954 | status | 50% | Same pattern as above |
...
### Data Set 2 · ARP4983_TAL_20241112.XLSM
...
|---|---|---|
| asset.SLGT003028xx.serial_number (×14) | 70% | 2 (value or empty) |
| asset.SLGT003028xx.status (×14) | 90% | 2 (value or empty) |
...
### Data Set 4 · TALNRL15205_20230118142012.XLSM
...
| Asset | Field | Match Rate | Values observed |
|---|---|---|---|
| POLE00985413 | serial_number | 71.4% | `PL985413` (runs 1, 5) · empty (runs 2, 3, 4, 6, 7) |

**Recommendation:** Add column alias for `serial_number` to cover common pole spreadsheet column names.

---

### Data Set 5 · TALNRL15237_20230113140532.XLSM
...
|---|---|---|---|
| BBLV00037149 | asset_type | 90% | `LV Busbar` (9 runs), `SWITCHGEAR` (run 6) |
| BBLV00037149 | status | 90% | `Outage Dependant Obj` (9 runs), empty (run 5) |
| (global) | rejected_records | 50% | `0` (runs 1, 2, 5, 6, 9) · `3` (runs 3, 4, 7, 8, 10) |

**BBLV00037149 asset_type:** Run 6 returns the sheet name (`SWITCHGEAR`) as the asset type instead of the cell value (`LV Busbar`). This is a single-run classification error where the LLM confuses the worksheet context with the field value.
...
### Data Set 6 · URS35318_TAL_20250515_final.XLSM

**Score: 93.3% (58.8/63 fields consistent across 10 runs)**

**Inconsistent fields: 10** — all concentrated on two ASSM pillar assets and global record-count fields:

| Asset/Field | Field | Match Rate | Pattern |
|---|---|---|---|
| ASSM00749529 | asset_id | 50% | Value in runs 1, 3, 5, 7, 10 · empty in runs 2, 4, 6, 8, 9 |
| ASSM00749529 | asset_type | 50% | `Pillars` in runs 1, 3, 5, 7, 10 · empty otherwise |
| ASSM00749529 | description | 50% | `Gen, UG Termination` in runs 1, 3, 5, 7, 10 · empty otherwise |
| ASSM00749529 | location | 70% | `Hoxton Park FieldServiceCentre` in runs 5, 7, 10 · empty otherwise |
| ASSM00749530 | asset_id | 50% | Same pattern as ASSM00749529 |
| ASSM00749530 | asset_type | 50% | Same pattern as ASSM00749529 |
| ASSM00749530 | description | 50% | Same pattern as ASSM00749529 |
| ASSM00749530 | location | 70% | Same pattern as ASSM00749529 |
| (global) | total_assets | 50% | `4` (runs 1, 3, 5, 7, 10) · `2` (runs 2, 4, 6, 8, 9) |
| (global) | rejected_records | 90% | `0` (9 runs) · `1` (run 2 only) |

**Pattern:** The two ASSM assets (`Gen, UG Termination` pillar type) are either both extracted or both entirely missing — never one without the other. Runs 1, 3, 5, 7, 10 extract all four assets (total = 4); runs 2, 4, 6, 8, 9 extract only the other two assets (total = 2). This 50/50 split across alternating runs indicates the LLM intermittently fails to read these two specific rows from the pillar worksheet, likely because `Gen, UG Termination` is an atypical description format that creates uncertainty about whether the row is a valid asset record.
...
## Single Prompt Baseline — Data Set 1 (10 Runs)

**Dataset:** Data Set 1 — Asset Relocation, 2A Windsor Road, McGraths Hill
...
- **Data Set 6 achieves perfect site plan score** — First dataset to score 100% on site plan extraction with 10 runs and fuzzy matching active.

### Remaining inconsistencies

| Issue | Scope | Impact | Fix |
|---|---|---|---|
| `X` status cell interpreted inconsistently | ARP5495 TAL, 2 assets | Low — semantically equivalent | Normalize `X` → `Marked for Deletion` in prompt |
| Serial number dropout, systematic | ARP4983 TAL, 14 assets | Medium — 30% of runs empty | Add column alias for `serial_number` |
...
| ASSM pillar rows intermittently missed | Data Set 6 TAL, 2 assets | Medium — 50% dropout on both assets | Prompt: all rows with populated asset ID are valid records |

---

## Consistency Score Summary

| Step | Data Set 1 | Data Set 2 | Data Set 4 | Data Set 5 | Data Set 6 |
|---|---|---|---|---|---|
