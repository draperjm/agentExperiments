    ...
    # F. DOCUMENT EXTRACTOR AGENT (Port 8090)
        ...
        if step.parallel_group:
            ...
            def _run_group_step(ps):
                ...
                # Multi-run mode: execute the same step N times with identical input
                all_run_results = []
                for run_idx in range(repeat_runs):
                    result = _strip_legend_extra_fields(_run_extractor_step(state, ps))
        ...
        # E2. Single extractor step
            ...
            if repeat_runs > 1:
                # Multi-run mode: execute N times with identical input
                log_message += f"   Multi-run mode: executing {repeat_runs} runs with identical input...\n"
                all_run_results = []
                
                async def _execute_run(run_number: int) -> dict:
                ...
                for run_idx in range(repeat_runs):
                    run_data = await _execute_run(run_idx + 1)
    ...
    # G. DATA ANALYTICS AGENT (Port 8092)
        ...
        if not step_failed and "match" in step_name_lower and "symbol" in step_name_lower:
            ...
            if site_plan_manifest:
                ...
                for _try in range(MAX_STEP_RETRIES + 1):
                    try:
                        resp = await _async_post(DOC_EXTRACTOR_URL, json=extractor_payload, timeout=600)
                        if resp.status_code == 200:
                            res_json = resp.json()
                            ...
                            if needs_legend and legend_entries:
                                ...
                                try:
                                    _leg_resp = await _async_post(ANALYTICS_URL, json=legend_payload, timeout=120)
...
_MANAGED_SERVICES = [
    {"name": "agent07-responder",          "label": "Responder",         "health": "http://responder:8000/tasks"},
    {"name": "agent07-orchestrator",       "label": "Orchestrator",      "health": None},
    {"name": "agent07-document-reviewer",  "label": "Document Reviewer", "health": "http://document-reviewer:8089/health"},
    {"name": "agent07-document-extractor", "label": "Document Extractor","health": "http://document-extractor:8090/health"},
    {"name": "agent07-step-validator",     "label": "Step Validator",    "health": "http://step-validator:8088/health"},
    {"name": "agent07-document-chunker",   "label": "Document Chunker",  "health": "http://document-chunker:8091/health"},
    {"name": "agent07-frontend",           "label": "Frontend",          "health": None},
]
